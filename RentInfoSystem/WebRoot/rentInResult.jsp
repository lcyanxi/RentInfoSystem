...
<script type="text/javascript">
	...
	function login() {
        $.ajax({
            ...
            url: "checkIn.action",
            data: {
                "inId": $.trim($("[type = 'radio']:checked").prop("name")),
...
<body>
			...
			<div class="header_top-sec">
				...
				<div class="top_left">
					<ul>
					   ...
					   <li class="top_link"> <c:if test="${session.username!=null}"><img alt="我的头像" src="Image/${session.head_Image}" width="50px" height="50px"></c:if></li>
		...
		<div class="container">
			<div class="menu_sec">
				...
				<ul class="megamenu skyblue">
					...
					<li class="grid"><a class="color2" href="rentIn.jsp">求租申请</a>
					</li>
					<li><a class="color4" href="rentOut.jsp">出租申请</a></li>
					<li><a class="color7" href="showRentOutData.action">出租结果</a>
					</li>
					<li><a class="color5" href="userInfo.action">个人中心</a>
					</li>
	...
	<div class="container">
		<ol class="breadcrumb">
			...
			<li class="active">求租结果</li>
		</ol>
		<div class="registration">
			<div class="registration_left">
				<h2>
					求租史? <span><a href="rentIn.jsp">求租申请</a> </span>
				</h2>
				<div class="menu_sec" style="width: 1150px">
					<table>
						...
						<c:forEach var="entry" items="${list}" varStatus="status">
							<tr>
								<td><img src="<%=path%>/Image/${entry.user.head_Image}"
					...
					<c:choose>
						...
						<c:otherwise>
					       <td><input type="radio" name=${entry.in_id} value="Y"onclick="disp_confirm()" />Y 
						   <input type="radio" name=${entry.in_id} value="N" onclick="disp_confirm()" />N</td>
					   </c:otherwise>
					</c:choose>
																													
								<td><a href="deleteRentIn.action?inId=${entry.in_id}" ><font color="#008080">删除</font>
								</a></td>
							</tr>
						</c:forEach>
						<tr>
							<c:choose>
								<c:when test="${page.hasPrePage }">
									<th><a href="showRentInData.action?currentPage=1">首页</a></th>
									<th><a
										href="showRentInData.action?currentPage=${page.currentPage-1}">上一页</a>
									</th>
							...
							<c:choose>
								<c:when test="${page.hasNextPage}">
									<td colspan="2"><a
										href="showRentInData.action?currentPage=${page.currentPage+1}">下一页</a>
									</td>
									<th colspan="2">总页数${page.totalPage}</th>
									<th align="center"><a
										href="showRentInData.action?currentPage=${page.totalPage}">尾页</a>
									</th>
								</c:when>
								<c:otherwise>
									<td>下一页</td>
